...
- **后台**：管理员登录、公告在线发布（可随附文档/表格/图片等附件）
...
| **管理员（登录后）** | 同上 + 管理后台 | 公告发布/编辑/删除，附件上传/删除 |
...
发布或编辑公告时，在表单底部「附件」一栏选择文件即可（可一次选多个）。
...
**前台展示**：公告详情页中，图片附件直接内联展示大图（点击可看原图），
其余格式以列表形式提供下载。

**编辑公告**：可单独删除某个已有附件（点附件右侧「删除」即时生效），
也可继续追加新附件。

> 说明：附件存放在非公开目录 `data/uploads/`，**不通过静态服务直接暴露**。
> 下载一律经 `/api/attachments/[id]` 校验后读取，并带
> `X-Content-Type-Options: nosniff`；非图片一律以「下载」方式返回，
> 不在浏览器内联渲染。
>
> 出于安全考虑，`.html`、`.svg`、`.js` 等可在浏览器中执行或嵌入脚本的
> 格式**不在允许范围内**，避免上传后被当作网页渲染造成 XSS。
...
better-sqlite3（数据库）、bcryptjs（密码哈希）、Node.js v22。
...
└── src/
    ...
    │   ├── attachments.ts      附件服务端校验（类型白名单、随机命名）
