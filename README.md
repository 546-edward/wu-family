...
```ts
theme: {
  primary: '#8B2E2E',   // 朱红，用于标题、强调、喜庆主调
  ink:     '#2B2B2B',   // 墨黑，用于正文与世系树连线
  paper:   '#F5F0E6',   // 米黄，用于宣纸底色
  accent:  '#B8860B',   // 赭金，用于分隔线、角饰、印章点缀
},
```
...
由 8 层 `background-image` 叠加而成：

| 层 | 内容 | 作用 |
|---|---|---|
| 1 | 纸色基底 `--theme-paper` | 宣纸米黄基调 |
| 2–3 | `repeating-linear-gradient` ×2 | 极细斜向条纹，模拟宣纸纤维 |
| 4 | 内联 SVG 回纹（万字不到头） | 中式暗格肌理，透明度仅 5.5% |
| 5–8 | `radial-gradient` ×4 | 四角朱红晕染，营造装裱感 |
| — | `radial-gradient` 中央提亮 | 保证正文区亮度与可读性 |

`background-attachment: fixed`，滚动时纸纹不跟随移动。

### 装饰元素

页面内容包裹在半透明宣纸面板 `.paper-panel` 中（占宽 80%），
两侧露出底色纹理；以下工具类可直接复用：

| 类名 | 效果 |
|---|---|
| `.paper-panel` | 半透明宣纸面板 + 淡金描边 |
| `.rule-festive` | 金—朱红—金 渐变分隔线 |
| `.seal-tag` | 朱红印章式标签（用于「堂号」「置顶」） |
| `.card-cn` / `.card-cn-hover` | 淡金描边卡片 + 悬停金边加深 |

首页首屏四角另有一组金色回纹角饰（内联 SVG，跟随 `--theme-accent`）。
导航栏顶部有一条朱红渐变细条。

### 可读性

底色叠加强度经过控制，实测正文对比度 **12.5:1**、标题 **7.3:1**，
